// MALBIT v24 · home hierarchy, translated feedback, themes, and expedition clarity.
(function(){
'use strict';
...
function patchHome(){
  ...
  document.querySelectorAll('.tqLongPressDiscovery').forEach(node=>node.remove());
...
function patchMore(){
  ...
  const badge=document.querySelector('.malbitPageTitle>span');if(badge)badge.textContent='v24';
...
function patchBattle(){
  ...
  if(activeBattle&&!screen.querySelector('.t1FullscreenCombatFx')){const fx=document.createElement('div'),kind=activeBattle.classList.contains('fx-player')?'player':activeBattle.classList.contains('fx-shield')?'shield':'enemy',impact=activeBattle.querySelector('.t1CombatImpact b')?.textContent||'';fx.className=`t1FullscreenCombatFx ${kind}`;fx.setAttribute('aria-hidden','true');fx.innerHTML=`<i></i><i></i><b>${escapeHtml(impact)}</b>`;screen.appendChild(fx)}
}
...
function postRender(){applyTheme();patchNavigationIcons();patchHome();patchMore();patchListeningNotice();patchGameHub();patchMap();patchBattle();patchRandomFeedback()}
...
const cleanupObserver=new MutationObserver(()=>{if(appState()?.view==='home')document.querySelectorAll('.tqLongPressDiscovery').forEach(node=>node.remove());patchListeningNotice()});
...
style.textContent=`
  ...
  .tqHomeScreen>.t1level{margin:12px 0 0!important}.tqV9HeroBottom{width:100%!important}.tqV9HeroBottom h2,.tqV9HeroBottom p{max-width:72%}.tqV9Continue{width:100%!important;min-height:52px;font-size:13px!important}
  ...
  .tqLongPressDiscovery{display:none!important}.audioSource:empty,.audioSource{display:none!important}
  ...
  @keyframes malbitSpark{50%{transform:rotate(12deg) scale(1.12);filter:drop-shadow(0 0 9px #a9e8ff) brightness(1.2)}}@keyframes v24ScreenFade{0%{opacity:0}22%{opacity:1}100%{opacity:0}}
  @media(max-width:380px){.tqV9HeroBottom h2,.tqV9HeroBottom p{max-width:78%}.t1TrailGrid{height:880px!important}.t1GameBattle{grid-template-columns:104px 1fr!important}.t1GameBattle .t1MonsterAvatar,.t1GameBattle .tqGameMonsterArt{width:102px!important;height:102px!important}.t1RunCurrencies{gap:3px}.t1RunCurrencies span{padding:5px 6px!important}}
  @media(prefers-reduced-motion:reduce){.malbitWordlight{animation:none}.t1TrailPawn{transition:none}.t1FullscreenCombatFx{animation-duration:.01ms}}
`;
