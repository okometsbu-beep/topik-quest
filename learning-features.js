...
window.malbitAddManualVocab=event=>{
  event?.preventDefault?.();const term=manualVocabTerm(document.getElementById('tqManualVocabTerm')?.value),note=String(document.getElementById('tqManualVocabMeaning')?.value||'').trim();
  if(!isSavableTerm(term))return toast(text('한국어 단어·숙어·문법을 입력해 주세요.','韓国語の単語・慣用句・文法を入力してください。','Enter a Korean word, idiom, or grammar form.','请输入韩语单词、惯用语或语法。')),false;
  if(!storeVocab({text:term,source:manualSource(),note,show:!!note,manual:true}))return toast(text('이미 단어장에 있어요.','すでに単語帳にあります。','Already in your vocabulary.','已经在单词本中。')),false;
  toast(text(`“${term}” 직접 추가 완료.`,`「${term}」を追加しました。`,`Added “${term}”.`,`已添加“${term}”。`));render();return false;
};
