...
function sourceLabel(){return read().engine==='neural'&&neuralInstalled()?'✨ LOCAL AI TTS':'⚠ DEVICE TTS FALLBACK'}
...
function settingsMarkup(){
  const prefs=read(),neural=window.MALBIT_NEURAL_TTS,installed=neuralInstalled(),aiRows=(neural?.voices||[]).map(voice=>`<button type="button" class="malbitTtsVoice ${prefs.neuralVoice===voice.id?'on':''}" data-tts-neural-voice="${voice.id}" onclick="malbitTtsChooseNeuralVoice(this.dataset.ttsNeuralVoice)"><span><b>${H(neuralVoiceName(voice))}</b><small>${L('신경망으로 기기에서 생성','端末内のニューラル生成','Neural, generated on device','神经网络在设备上生成')}</small></span><i>▶</i></button>`).join('');
  const packAction=installed?`<button type="button" class="malbitTtsRemovePack" onclick="malbitTtsRemoveNeural()">${L('AI 음성팩 삭제','AI音声パックを削除','Remove AI voice pack','删除AI语音包')}</button>`:`<button type="button" class="malbitTtsInstallPack" onclick="malbitTtsInstallNeural()">↓ ${L('무료 AI 음성팩 받기 · 약 230MB','無料AI音声パックを取得 · 約230MB','Download free AI voice pack · about 230 MB','下载免费AI语音包 · 约230MB')}</button>`;
  return`<section class="malbitSetting malbitTtsSetting"><h2>${L('한국어 음성·속도','韓国語の音声・速度','Korean voice & speed','韩语音色与速度')}</h2><p>${L('여기서 한 번 고르면 문제·단어장·스토리·말하기에 모두 적용됩니다.','ここで一度選ぶと、問題・単語帳・ストーリー・スピーキングのすべてに反映されます。','Choose once here to apply it to questions, vocabulary, stories, and speaking.','在此设置一次，即可应用到题目、单词本、故事和口语。')}</p><div class="malbitTtsEngines"><button type="button" class="${prefs.engine==='neural'?'on':''}" data-tts-engine="neural" onclick="malbitTtsChooseEngine('neural')"><b>✨ ${L('무료 AI 음성','無料AI音声','Free AI voice','免费AI语音')}</b><small>${L('사용료 0원 · 문장을 기기 안에서 생성','利用料0円 · 端末内で生成','No usage fee · generated on device','零使用费 · 在设备上生成')}</small></button><button type="button" class="${prefs.engine==='device'?'on':''}" data-tts-engine="device" onclick="malbitTtsChooseEngine('device')"><b>⚡ ${L('기기 음성','端末音声','Device voice','设备语音')}</b><small>${L('다운로드 없이 바로 재생','ダウンロードなしですぐ再生','Instant, no download','无需下载，立即播放')}</small></button></div><div class="malbitTtsPack ${installed?'installed':''}"><div><b data-tts-pack-title>${installed?L('AI 음성팩 준비 완료','AI音声パック準備完了','AI voice pack ready','AI语音包已就绪'):L('처음 한 번만 다운로드','初回のみダウンロード','One-time download','仅首次下载')}</b><small data-tts-pack-status>${installed?L('10가지 음성 · 별도 결제 없음','10種類の音声 · 追加料金なし','10 voices · no extra charge','10种音色 · 无额外费用'):L('10가지 음성 · Wi-Fi 권장 · 받는 동안 기존 음성 사용 가능','10種類の音声 · Wi-Fi推奨 · 取得中も端末音声を使用可能','10 voices · Wi-Fi recommended · device speech remains available','10种音色 · 建议使用Wi-Fi · 下载时仍可用设备语音')}</small></div>${packAction}<progress class="malbitTtsPackProgress" max="100" value="0" hidden></progress></div><div class="malbitTtsVoices malbitTtsAiVoices">${aiRows}</div><details class="malbitTtsDeviceDetails" ${prefs.engine==='device'?'open':''}><summary>${L('기기에 설치된 음성 보기','端末にインストール済みの音声','Installed device voices','查看设备已安装音色')}</summary><div class="malbitTtsVoices">${deviceRows(prefs)}</div></details><div class="malbitTtsSpeedHead"><b>${L('음성 속도','読み上げ速度','Voice speed','朗读速度')}</b><output data-tts-rate>${clampRate(prefs.rate).toFixed(2)}×</output></div><input class="malbitTtsRange" type="range" min="0.65" max="1.05" step="0.01" value="${clampRate(prefs.rate)}" aria-label="${L('음성 속도','読み上げ速度','Voice speed','朗读速度')}" oninput="malbitTtsSetRate(this.value,false)" onchange="malbitTtsPreview()"><div class="malbitTtsPresets"><button type="button" data-tts-rate-value="0.72" class="${clampRate(prefs.rate)===.72?'on':''}" onclick="malbitTtsSetRate(.72,true)">${L('천천히','ゆっくり','Slow','慢速')}<small>0.72×</small></button><button type="button" data-tts-rate-value="0.82" class="${clampRate(prefs.rate)===.82?'on':''}" onclick="malbitTtsSetRate(.82,true)">${L('학습용','学習向け','Learning','学习')}<small>0.82×</small></button><button type="button" data-tts-rate-value="0.92" class="${clampRate(prefs.rate)===.92?'on':''}" onclick="malbitTtsSetRate(.92,true)">${L('보통','標準','Normal','正常')}<small>0.92×</small></button></div><button type="button" class="malbitTtsPreview" onclick="malbitTtsPreview()">🔊 ${L('현재 설정으로 듣기','現在の設定で聞く','Play current settings','按当前设置试听')}</button><blockquote>“${SAMPLE}”</blockquote><p class="malbitTtsLicense">${L('AI 음성은 Supertonic 3 기반이며, 문장은 서버로 보내지지 않습니다.','AI音声はSupertonic 3を使用し、文章はサーバーへ送信されません。','AI speech uses Supertonic 3; your text is not sent to a server.','AI语音基于Supertonic 3，文本不会发送到服务器。')} <a href="https://huggingface.co/Supertone/supertonic-3" target="_blank" rel="noopener">${L('모델·라이선스','モデル・ライセンス','Model & license','模型与许可')}</a></p></section>`;
}
...
window.malbitTtsChooseEngine=value=>{const prefs=read();prefs.engine=value==='device'?'device':'neural';write(prefs);refreshSettings();if(prefs.engine==='device')play(SAMPLE);else if(neuralInstalled())play(SAMPLE);else announce(L('아래 버튼으로 무료 AI 음성팩을 먼저 받아 주세요.','下のボタンから無料AI音声パックを取得してください。','Download the free AI voice pack below first.','请先点击下方按钮下载免费AI语音包。'))};
window.malbitTtsChooseNeuralVoice=value=>{const prefs=read();prefs.engine='neural';prefs.neuralVoice=/^[FM][1-5]$/.test(value)?value:'F2';write(prefs);refreshSettings();if(neuralInstalled())play(SAMPLE);else announce(L('AI 음색을 저장했습니다. 음성팩을 받으면 바로 들을 수 있어요.','AI音色を保存しました。音声パック取得後に再生できます。','AI voice saved. Download the pack to hear it.','AI音色已保存，下载语音包后即可试听。'))};
...
window.malbitTtsPreview=()=>{const prefs=read();if(prefs.engine==='neural'&&!neuralInstalled()){announce(L('무료 AI 음성팩을 먼저 받아 주세요.','無料AI音声パックを先に取得してください。','Download the free AI voice pack first.','请先下载免费AI语音包。'));return Promise.resolve()}return play(SAMPLE)};
window.malbitTtsInstallNeural=async()=>{
  const neural=window.MALBIT_NEURAL_TTS;if(!neural)return;neural.unlockAudio?.();const prefs=read();prefs.engine='neural';write(prefs);refreshSettings();let downloaded=false;
