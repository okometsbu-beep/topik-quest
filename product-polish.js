...
function appVersion(){return String(window.__MALBIT_RUNTIME__?.version||'')}
...
function renderVocabCard(v,index){const target=vocabTarget(),meaning=v.meanings?.[target]||(target==='ja'?v.ja:'')||'',due=Number(v.dueAt)<=Date.now();return`<article class="malbitVocabCard ${due?'due':''}" data-vocab-index="${index}"><div class="malbitVocabTop"><span>${due?'●':'○'} ${dueLabel(v)}</span><small>${html(v.source||L('직접 저장','手動保存','Saved','已收藏'))}</small></div><h2 lang="ko">${html(v.text)}</h2><div class="malbitVocabMeaning ${v.show?'show':''}">${v.show?html(meaning||L('뜻을 불러오는 중…','意味を読み込み中…','Loading meaning…','正在加载释义…')):L('먼저 뜻을 떠올린 뒤 확인하세요.','意味を思い出してから確認しましょう。','Recall the meaning before revealing it.','先回想词义，再点击查看。')}</div><div class="malbitVocabActions"><button class="reveal" onclick="malbitVocabReveal(${index})">${v.show?L('뜻 숨기기','意味を隠す','Hide','隐藏'):L('뜻 확인','意味を確認','Reveal','查看释义')}</button><button class="sound" onclick='malbitSpeak(${JSON.stringify(v.text)})'>🔊</button><button class="remove" onclick="deleteVocab(${index})">×</button></div>${v.show?`<div class="malbitSrs"><small>${L('다시 언제 볼까요?','次はいつ復習しますか？','When should this return?','下次何时再复习？')}</small><div><button onclick="malbitVocabGrade(${index},'hard')">${L('어려움','難しい','Hard','困难')}<b>10m</b></button><button onclick="malbitVocabGrade(${index},'good')">${L('보통','普通','Good','一般')}<b>${Math.max(1,Math.round(Number(v.interval)||1))}d</b></button><button onclick="malbitVocabGrade(${index},'easy')">${L('쉬움','簡単','Easy','简单')}<b>${Math.max(3,Math.round((Number(v.interval)||1)*3))}d</b></button></div></div>`:''}<button class="malbitVocabDetailLink" onclick="malbitOpenVocabEditor(${index})"><span>${L('상세 보기·편집','詳細を見る・編集','Open details & edit','查看详情并编辑')}</span><b>›</b></button></article>`}
...
const PORTABLE_KEYS=['topikQuestV8','topikQuestExamLevel',T1_SESSION,SHORTS_KEY,GAME_KEY,REVIEW_KEY,EVENTS_KEY,PREFS_KEY,ONBOARD_KEY,'malbitBeginnerV1','malbitStoryV1','malbitTtsPrefsV1','malbitDiagnosticV1','malbitJourneyEventsV1','malbitGrowthPrefsV1','malbitInstallIdV1','malbitShortProposalHandled'];
