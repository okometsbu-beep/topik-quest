...
test('Shorts audit covers the actual deck and keeps all content approvals unreviewed', () => {
  ...
  assert.equal(report.summary.total, 332);
  assert.deepEqual(report.summary.byLevel, [{level:1,total:192,curated:68,bank:124},{level:2,total:140,curated:78,bank:62}]);
  assert.equal(new Set(report.rows.map(row => row.id)).size, 332);
  assert.equal(report.summary.approved, 0);
  assert.equal(report.summary.distinctQuestionChoiceSets, 206);
