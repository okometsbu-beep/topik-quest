...
test('Shorts audit covers the actual deck and keeps all content approvals unreviewed', () => {
  ...
  assert.equal(report.summary.total, 316);
  assert.deepEqual(report.summary.byLevel, [{level:1,total:184,curated:60,bank:124},{level:2,total:132,curated:70,bank:62}]);
  assert.equal(new Set(report.rows.map(row => row.id)).size, 316);
  assert.equal(report.summary.approved, 0);
  assert.equal(report.summary.distinctQuestionChoiceSets, 190);
