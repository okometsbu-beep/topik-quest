...
test('Shorts audit covers the actual deck and keeps all content approvals unreviewed', () => {
  ...
  assert.deepEqual(report.summary.byLevel, [{level:1,total:176,curated:52,bank:124},{level:2,total:124,curated:62,bank:62}]);
  assert.equal(new Set(report.rows.map(row => row.id)).size, 300);
  assert.equal(report.summary.approved, 0);
  assert.equal(report.summary.distinctQuestionChoiceSets, 174);
