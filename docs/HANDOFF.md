...
## Current release priority · 2026-09-21
...
- Production is v128 at `6840bd478937ebb524f59ec8b68c203f41a1fb46` (PR #166), adding four
  bounded TOPIK II scope-relation cards for exclusion, role substitution, an irrelevant condition, and
  representative inclusion. Production inventory is 392 rows / 266 exact families (I 132, II 134);
  local focused vocabulary+inventory checks pass 31/31, content checks 34/34, and the full release check
  128/128 with the v128 45-file runtime contract. PR #166 evidence CI `35556820642` and final CI
  `35557160438` cover Linux Chrome 320/375/390/430px light/dark, selected-wrong feedback, expanded
  coaching, next-first flow, and reload restoration. Artifact `10621001103` screens `00dg`/`00dh` were
  inspected. Main CI `35557469515` and Pages `35557469072` succeeded. Live HTTP reports v128;
  `index.html`, `site-patch.js`, `sw.js`, and `data/shorts-levels.js` hashes match main and all four new
  IDs are present. Public Chrome verified TOPIK II Shorts entry, the new `-에 관계없이` card, deliberate
  ...
  Review, recovery, and compatible legacy migration. Product rollback is v127
  `b1d9140dc9b06ecfd63bbf92dfbdaf8d056a9ac5`. Physical devices, native review, learner timing/recall,
  audio, and full offline recovery remain unverified. The bounded ledger is
  `docs/qa/shorts-review-s04-topik2-scope-relations.md`.
- Candidate v129 adds four stable TOPIK I transit-action IDs for boarding (`타다`), exiting
...
- Latest #110 instruction: C01–C06 content quality, first applied as S01–S05 Shorts, outranks A08/#76.
  ...
  public-browser verification passed; product rollback is v127. Candidate v129 now adds four bounded
  TOPIK I transit-action sets; after deployment, the next task is to synchronize its production evidence.
