...
- 첫 push CI 36227898870에서 발견한 넓은 RPG 제약을 수정했다. 두 번째 CI
  36228119142는 통과했고 223장 중 새 홈/학습/내 기록 및 기존 학습 화면을 직접 확인했다.
- CI 36228355419는 카메라 초기 배치의 다음 프레임 전 애니메이션 속성을 읽어 실패했다.
  임시 transition 해제까지 제한된 대기를 추가했으며 기존 이동/보간 합격 조건은 유지한다.
- 최종 후보는 단어장·말하기까지 6개 화면×4개 폭×두 테마의 실제 메뉴 이동·44px 터치·
  넘침·언어·콘솔 검사를 포함한다. 최종 CI/배포 증거는 PR 및 #110 완료 기록을 따른다.
