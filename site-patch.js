...
// Developer Listening Studio: persistent GitHub connection + one microphone grant per studio session.
(function(){
  'use strict';
  ...
  window.saveDevGithubToken=async function(){
    ...
    if(el)el.value='';
    toast('이 기기에 연결을 기억했습니다. 다음 접속부터 자동 복구합니다.');
    render();
  };
  ...
  window.playDevRecordingPreview=function(id){
    ...
    stopDevPreview();
    try{const a=new Audio(x.url);devPreviewAudio=a;a.onended=()=>{devPreviewAudio=null};a.onerror=()=>{devPreviewAudio=null;toast('녹음 미리듣기에 실패했습니다.')};a.play()}catch(e){toast('녹음 미리듣기에 실패했습니다.')}
  };
  window.playUploadedDevAudio=async function(id){
    stopDevPreview();stopListeningAudio();const q=String(id).padStart(3,'0');
    for(const ext of AUDIO_EXTS){
      const url=`audio/topik2/q${q}.${ext}?v=${Date.now()}`;
      try{const r=await fetch(url,{method:'HEAD',cache:'no-store'});if(!r.ok)continue;const a=new Audio(url);devPreviewAudio=a;a.onended=()=>{devPreviewAudio=null};a.onerror=()=>{devPreviewAudio=null;toast('업로드된 음성을 재생하지 못했습니다.')};await a.play();return}catch(e){}
    }
  ...
  window.uploadDevRecording=async function(id){
    ...
    stopDevPreview();const q=String(id).padStart(3,'0');const targetPath=`audio/topik2/q${q}.${x.ext}`;
    devUploadStatus[id]={state:'uploading',text:'기존 음원 확인 중…'};render();
    try{
      const existing=[];
      for(const ext of AUDIO_EXTS){const path=`audio/topik2/q${q}.${ext}`;const file=await currentGithubFile(path,token);if(file?.sha)existing.push({path,sha:file.sha})}
      const targetOld=existing.find(f=>f.path===targetPath);const content=await blobBase64(x.blob);
      ...
      let data={};try{data=await r.json()}catch(e){};if(!r.ok)throw new Error(data.message||`GitHub PUT ${r.status}`);
      ...
      console.error(e);devUploadStatus[id]={state:'error',text:'업로드 실패'};const msg=String(e?.message||e);
      if(/401|Bad credentials|Requires authentication/i.test(msg)){memoryToken='';try{localStorage.removeItem(TOKEN_KEY)}catch(_){ }try{await idbDel(TOKEN_KEY)}catch(_){ }toast('GitHub 권한이 만료되었거나 잘못되었습니다. 새 토큰이 필요합니다.')}else if(/403|permission|Resource not accessible/i.test(msg)){toast('GitHub token의 Contents 쓰기 권한을 확인해 주세요.')}else toast('업로드 실패: '+msg.slice(0,100));render();
    }
  };
  ...
  window.prepareDevMicrophone=async function(){
    ...
    try{
      ...
      render();return true;
    }catch(e){console.error(e);toast('마이크 권한을 허용해 주세요. Safari 사이트 설정에서 마이크를 허용으로 고정할 수 있습니다.');return false}
  };
  window.releaseDevMicrophone=function(){
    try{devStream?.getTracks().forEach(t=>t.stop())}catch(e){};devStream=null;toast('마이크를 껐습니다.');render();
  };
  ...
  window.toggleDevRecord=async function(id){
    ...
    try{
      const types=['audio/mp4','audio/webm;codecs=opus','audio/webm','audio/ogg;codecs=opus'];const mime=types.find(t=>MediaRecorder.isTypeSupported?.(t))||'';
      ...
      devRecorder.onstop=()=>{
        const type=devRecorder.mimeType||mime||'audio/webm';const ext=type.includes('mp4')?'m4a':type.includes('ogg')?'ogg':'webm';const blob=new Blob(devChunks,{type});const url=URL.createObjectURL(blob);
        if(devBlobs[id]?.url)try{URL.revokeObjectURL(devBlobs[id].url)}catch(e){}
        devBlobs[id]={blob,url,ext};devUploadStatus[id]={state:'ready',text:'녹음 완료 · 미리듣기 후 업로드 가능'};devRecorder=null;devRecordingId=null;
  ...
  window.developerStudio=function(sc){
    navActive('more');const linked=!!ghToken();const micReady=micAlive();
    const rows=LS.map(q=>{
      ...
      else{action=`<button id="devRec_${q.id}" onclick="toggleDevRecord(${q.id})">🎙 ${x?'다시 녹음':'녹음'}</button>`;if(x){action+=`<button onclick="playDevRecordingPreview(${q.id})">▶ 내 녹음</button><button onclick="uploadDevRecording(${q.id})" ${st?.state==='uploading'?'disabled':''} style="${st?.state==='done'?'background:#143d31;border-color:#2c8a69;color:#d9fff1':''}">${st?.state==='uploading'?'⏳ 처리 중':st?.state==='done'?'↻ 다시 교체':'⬆ 업로드/교체'}</button>`}}
    ...
    sc.innerHTML=`
      ...
      <div class="devBanner">${ml('한 번 연결하면 같은 Safari/Web App에서 GitHub 권한을 자동 복구합니다. 마이크는 제작실을 열어 둔 동안 한 번만 허용하고 계속 재사용합니다.','一度接続すると同じSafari/Web AppでGitHub権限を自動復元します。マイクは制作室を開いている間、一度だけ許可して再利用します。','GitHub access is restored automatically in the same Safari/Web App. The microphone is requested once per studio session and reused.','同一 Safari/Web App 会自动恢复 GitHub 权限。麦克风在制作室会话中只请求一次并持续复用。')}</div>
      <div class="infoCard"><h3>📱 iPhone 권장 설정</h3><p>ChatGPT 안의 임시 브라우저보다 <b>Safari</b>에서 이 제작실을 열거나 홈 화면에 Web App으로 추가하는 것을 권장합니다. Safari에서 이 사이트의 마이크를 <b>허용</b>으로 설정하면 재접속 시 권한 질문을 줄일 수 있습니다.</p><button class="primary alt" onclick="copyDevStudioUrl()">제작실 주소 복사</button></div>
      ...
      <div class="infoCard"><h3>🔑 GitHub 자동 업로드</h3>${linked?`<p>✓ 연결 기억됨. 다시 토큰을 만들거나 붙일 필요 없습니다.</p><button class="primary alt" onclick="clearDevGithubToken()">연결 해제</button>`:`<p>최초 한 번만 fine-grained token을 입력합니다. 이후 같은 Safari/Web App에서는 localStorage와 IndexedDB에서 자동 복구합니다.</p><input id="devGhToken" type="password" autocomplete="off" placeholder="github_pat_..." style="width:100%;padding:13px;border-radius:13px;border:1px solid #314663;background:#0c192b;color:#fff;margin:8px 0"><div style="display:grid;grid-template-columns:1fr 1fr;gap:7px"><button class="primary alt" onclick="openGithubTokenSetup()">GitHub 권한 만들기</button><button class="primary" onclick="saveDevGithubToken()">이 기기에 연결</button></div><p style="margin-top:8px;font-size:9px">topik-quest만 선택 · Repository permissions → Contents: Read and write.</p>`}</div>
